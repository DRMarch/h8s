...
| **Auth** | None on the Service; exposed via Higress at `/mcp/servers/searxng` behind the domain-level `key-auth` |
| **Limiter** | Already exempt — the pod's CIDR (`10.0.0.0/8` default) is in the limiter `pass_ip` list |

The gateway registration lives in `networking/higress/resources/mcp/servers/searxng/` (see [`networking/higress/README.md`](../../networking/higress/README.md#mcp-portal)).
