...
# ---- Vault Push: OpenRouter API Key for Higress ----
#
# Supplied as a bring-your-own variable rather than generated by Terraform.
# The Higress ai-proxy WasmPlugin references the resulting ESO Secret with
# ${secret.higress-system/higress-openrouter.api-key}.
#
# If the variable is empty, this resource is skipped so the workspace remains
# usable before the provider is configured.
resource "null_resource" "vault_openrouter_api_key" {
  count = trimspace(var.openrouter_api_key) == "" ? 0 : 1

  # Do not persist the raw API key in Terraform triggers/state. The hash only
  # detects rotation; the value is passed through the provisioner's environment
  # and a temporary stdin payload instead.
  triggers = {
    api_key_sha256 = sha256(var.openrouter_api_key)
  }
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      # jq performs JSON escaping; the key never becomes a shell command argument.
      jq -n --arg key "$OPENROUTER_API_KEY" '{"api-key":$key}' > "$TMP_JSON"

      # Send the payload over kubectl exec stdin, then let Vault read it from
      # a short-lived file inside the Vault pod. The raw key is not present
      # in the kubectl command line or Terraform resource triggers.
      kubectl exec -i ${var.vault_pod} -n ${var.vault_namespace} -- \
        /bin/sh -c 'cat > /tmp/openrouter.json' < "$TMP_JSON"

      VAULT_TOKEN=$(${local.vault_token_cmd})
      kubectl exec ${var.vault_pod} -n ${var.vault_namespace} -- /bin/sh -c \
        'export VAULT_TOKEN="$1"; vault kv put ${var.vault_kv_mount}/higress/openrouter @/tmp/openrouter.json; rm -f /tmp/openrouter.json' \
        /bin/sh "$VAULT_TOKEN"
    EOT
  }
}
